...
//========================================================================
QByteArray  Epos::Createwrist_command_packet(QList<int> motorPositions, int32_t id,int offset)
{
    QByteArray command;
...
//========================================================================
QByteArray  Epos::Createpalm_command_packet(QList<int> motorPositions, int32_t id, int cmd_id, bool right_hand)
{
    QByteArray command;
    command.append(id & 0xff);
    command.append((id>>8)&0xff);
    ...
    if(cmd_id == 10) { int offset = 0; }
    else if (cmd_id == 11) { int offset = 6; } 
    else if (cmd_id == 12) { int offset = 12; }
...
inline QByteArray Epos::CreatePalmCommand(QList<int> motorPositions)
{
    ...
    {
    case 0:
    command = Createwrist_command_packet(motorPositions, 0x281, 23);
    break;
    case 1:
    command = Createwrist_command_packet(motorPositions, 0x282, 26);
    if( (motorPositions[15+29] != 1 ) && (motorPositions[15+29] != 1) ) 
    {
        palmID = 3;
    }
    break;
    case 2:
     command = Createpalm_command_packet(motorPositions, 0x281, 10, true);
    break;
    case 3:
    command = Createpalm_command_packet(motorPositions, 0x282, 10, false);
    break;

    }
    palmID++;
    
    if(palmID==4)palmID = 0;
    ...
    //     if(subPalmID1 == 0)
    //     {   
    //         command = Createpalm_command_packet(motorPositions, 0x281, 11, true);

    //         subPalmID1++;
    //     }
    //     else if(subPalmID1 == 1)
    //     {
    //         command = Createpalm_command_packet(motorPositions, 0x281, 12, true);
    ...
    //     if(subPalmID2 == 0)
    //     {
    //         command = Createpalm_command_packet(motorPositions, 0x282, 11, false);
    //         subPalmID2++;
    //     }
    //     else if(subPalmID2 == 1)
    //     {
    //         command = Createpalm_command_packet(motorPositions, 0x282, 12, false);
    //         subPalmID2++;
    //     }
...
void Epos::SetAllPositionCST(QList<int> motorPositions)
{
    ...
    QByteArray command;
    for(int i=0; i< 12; i++)
        command.append(MotorDataToArray(0x401,motorPositions.at(i))); //72 bytes 
    command.append(CreateHandPacket(motorPositions)); //20 bytes
