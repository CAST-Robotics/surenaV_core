...
void Robot::runFootLenController(double f_l, double f_r, ControlState robot_cs)
{
    ...
    double delta_z = onlineWalk_->footLenController(deltaFd, floor((f_l - f_r) * 10) / 10, 0.00011, 0.0, 1.0);
...
void Robot::handMotion(double t_step, int step_count, double max_angle_deg, double dt, bool isLeftFirst, bool include_height_phases)
{
    ...
    int initial_pause_samples = include_height_phases ? 610 : 210; 
    // int initial_pause_samples = include_height_phases ? 410 : 10; for left first
    ...
    const int total_step_samples = swing_samples + double_support_samples;


    int total_samples = initial_pause_samples + (step_count * total_step_samples) + (include_height_phases ? 200 : 0); // Extra buffer only when including pauses
